...
const confirmUpdateDomainRecord = () => {
	...
	if (selectLine.value == null) {
		disabled.value = false
		return toast.add({ severity: 'error', summary: 'ERROR', detail: '解析参数不能为空', life: 3000 })
	}
...
<template>
	<div class="card">
		...
		<p>禁止搭建任何涉及支付的网站
			<br>
			禁止搭建如博彩、色情、赌博、诈骗等违法违规网站或进行其他违法违规行为
		</p>
	</div>
	<div>
		<div class="card">
			...
			<DataTable showGridlines :value="domainList" dataKey="id" columnResizeMode="fit" :paginator="true" :rows="10"
				...
				<Column field="line" header="解析参数" style="min-width: 8rem"></Column>
		...
		<Dialog v-model:visible="updateDomainRecordDialog" :style="{ width: '450px' }" header="更新记录" :modal="true"
			...
			<div class="field">
				<label for="line">解析参数</label>
				<Dropdown id="line" v-model="selectLine" :options="lines" optionLabel="name" placeholder="请选择解析解析参数"
					class="w-full" />
			</div>
			<div class="field">
