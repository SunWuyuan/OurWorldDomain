...
const userModel = ref([
    ...
    //{ label: '云服务', items: [ { label: '不限速云盘', icon: 'pi pi-fw pi-send', to: '/pan' }, { label: '云服务器', icon: 'pi pi-fw pi-globe', to: '/service' }, ] },
    { label: '其它', items: [
...
<template>
    <ul class="layout-menu" v-if="auth === '1'">
        <template v-for="(item, i) in adminModel" :key="item">
            <app-menu-item v-if="!item.separator" :item="item" :index="i"></app-menu-item>
            <li v-if="item.separator" class="menu-separator"></li>
        </template>
    </ul>
    <ul class="layout-menu" v-else>
        <template v-for="(item, i) in userModel" :key="item">
            <app-menu-item v-if="!item.separator" :item="item" :index="i"></app-menu-item>
            <li v-if="item.separator" class="menu-separator"></li>
        </template>
    </ul>
